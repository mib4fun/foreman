class Authorizer
  ...
  def build_filtered_scope_components(resource_class, all_filters, options)
    ...
    if all_filters.empty? || (!@base_collection.nil? && @base_collection.empty?)
      Rails.logger.debug 'no filters found for given permission' if all_filters.empty?
      Rails.logger.debug 'base collection of objects is empty' if !@base_collection.nil? && @base_collection.empty?
